...
struct PB{BYTE v,m;};
static HMODULE g_dll; static AssignFn g_assign;
...
static std::unordered_map<std::string,std::string> g_displayCache; static std::mutex g_lock;
...
static const char* trImpl(const char*s){if(!s||!*s)return s;std::string r(s);auto a=r.find_first_not_of(" \t"),b=r.find_last_not_of(" \t");if(a==r.npos)return s;std::string key=r.substr(a,b-a+1);auto i=g_dict.find(key);if(i==g_dict.end()){logMissing("CTOR",key.c_str(),key.size());return s;}std::lock_guard<std::mutex>l(g_lock);auto cached=g_displayCache.find(r);if(cached!=g_displayCache.end())return cached->second.c_str();std::string out=r.substr(0,a)+i->second+r.substr(b+1);auto inserted=g_displayCache.emplace(std::move(r),std::move(out));return inserted.first->second.c_str();}
